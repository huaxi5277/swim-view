...
class EditableTable extends React.Component {
  ...
  delete(form, key){
   ...
   axios.post('http://localhost:4000/swimmimg/physical/deletePhysical',obj)
  ...
  save(form, key) {
    form.validateFields((error, row) => {
      ...
      if (index > -1) {
        ...
        // 发起网络请求 
        axios.post('http://localhost:4000/swimmimg/physical/findphysical',newData[index])
...
const EditableFormTable = Form.create()(EditableTable);
 ...
 class index extends Component {
    ...
    handleSubmit(e){
        ...
        this.props.form.validateFields((err,values)=>{ 
            if(!err){
                const {text } = values
                obj.content  = text
                axios.post('http://localhost:4000/swimmimg/physical/add',obj)
   ...
   componentDidMount(){
       axios.get('http://localhost:4000/swimmimg/physical/all').then((result)=>{
          result.data.forEach((item,i)=>{
             if(item.title == '基础锻炼'){
                 this.setState({
                    renderTable : item.physical
                 })
             }
          })
