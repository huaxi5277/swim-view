...
class EditableTable extends React.Component {
  ...
  delete(form, key){
   ...
   axios.post('http://localhost:4000/swimmimg/beforeAndafter/deleteBefore',obj)
  ...
  save(form, key) {
    form.validateFields((error, row) => {
      ...
      if (index > -1) {
        ...
        // 发起网络请求 
        axios.post('http://localhost:4000/swimmimg/beforeAndafter/findbefore',newData[index])
...
const EditableFormTable = Form.create()(EditableTable);
 ...
 class index extends Component {
    ...
    handleSubmit(e){
        ...
        this.props.form.validateFields((err,values)=>{ 
            if(!err){
                const {text } = values
                obj.content  = text
                axios.post('http://localhost:4000/swimmimg/beforeAndafter/addB',obj)
   ...
   componentDidMount(){
       axios.get('http://localhost:4000/swimmimg/beforeAndafter/allB').then((result)=>{
          result.data.forEach((item,i)=>{
             if(item.title == '泳前'){
                 this.setState({
                    renderTable : item.before
                 })
             }
          })
